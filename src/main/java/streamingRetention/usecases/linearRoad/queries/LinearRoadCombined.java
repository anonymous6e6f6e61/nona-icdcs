...
public class LinearRoadCombined {
    public static void main(String[] args) throws Exception {
        ...
        DataStream<VehicleTupleGL> stopped =  sourceStream
...
//                .slotSharingGroup(settings.secondSlotSharingGroup())
        ...
        DataStream<CountTupleGL> accidents = sourceStream
                .filter(t -> t.getType() == 0 && t.getSpeed() == 0)
                .name("SpeedFilter"+settings.queryID())
                .keyBy(t -> t.getKey())
                .window(
...
//        .slotSharingGroup(settings.secondSlotSharingGroup())
        ...
        settings
                ...
                .activate(
                        Arrays.asList(
                                ProvenanceActivator.convert(accidents),
                                ProvenanceActivator.convert(stopped)
                        ),
                        Arrays.asList("ACCIDENTS", "STOPPED"),
                        settings,
                        settings.queryU(),
                        timestampConverter,
                        env.getConfig()
        ...
        env.execute("LinearRoadCombined queryID" + settings.queryID());
    }
}
